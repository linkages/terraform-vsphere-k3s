resource "null_resource" "control_plane_first_node" {
  provisioner "remote-exec" {
    inline = [
      "curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC='server --write-kubeconfig-mode 644 --cluster-init --token ${var.k3s_token} --tls-san ${var.lb_address}' INSTALL_K3S_VERSION='v1.21.1+k3s1' sh -"
    ]
...
resource "null_resource" "control_plane_subsequent_nodes" {
  ...
  provisioner "remote-exec" {
    inline = [
      "curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC='--write-kubeconfig-mode 644 --server https://${var.first_control_plane_node_ip}:6443 --token ${var.k3s_token}' INSTALL_K3S_VERSION='v1.21.1+k3s1' sh -"
    ]
